...
class AccountInvoice(models.Model):
    ...
    def refund_partial(self, date_invoice=None, date=None, description=None,
                       journal_id=None, lines_id=None, percentage=None):
        ...
        for invoice in self:
            ...
            values = self._prepare_refund_partial(invoice,
                                                  ...
                                                  lines_id=lines_id, percentage=percentage)
    ...
    def _prepare_refund_partial(self, invoice, date_invoice=None,
        ...
        values['invoice_line_ids'] = self.with_context(dict(self._context, refund_percentage=percentage))._refund_cleanup_lines(lines_id)
        if journal_id:
    ...
    def _refund_cleanup_lines(self, lines):
        ...
        if not result.get('sale_line_ids'):
            for line in lines:
                values = {}
                for name, field in line._fields.items():
                    if name == 'sale_line_ids':
                        result.append((0, 0, [(6, 0, line[name].ids)]))
        return result
